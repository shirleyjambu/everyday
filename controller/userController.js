...
const prepareSchedule = dbUser => {
  ...
  let fArr = sArr.filter(function(el) {
    return (
      moment(el.time).format("YYYYMMDD") ===
      moment(new Date(Date.now())).format("YYYYMMDD")
    );
  });
...
const removeEmptyValues = obj => {
  ...
  for (key in obj) {
    ...
    if (key === "buy") {
      fArr = sArr.filter(function(el) {
        return el != "";
      });

      obj[key] = fArr;
    } else {
      fArr = sArr.filter(element => element != "");
      console.log(fArr);
      obj[key] = fArr;
    }
  }
...
module.exports = {
  ...
  // Clear or Update items
  clear(req, res) {
    console.log("User Controller - Update or Clear");
    //console.log(req.body.user_id);
    //console.log(req.body.empObj);

    db.User.findOneAndUpdate(
      { _id: req.body.user_id },
      { $set: removeEmptyValues(req.body.empObj) }
    )
      .then(dbUser => res.json(dbUser))
      .catch(err => {
        console.log(err);
